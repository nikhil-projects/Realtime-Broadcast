...
↓↓←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←↓↓
↓↓→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→↓↓
↓↓      server.py  Created by  Durodola Opemipo 2019                     ↓↓
...
class OnyxServerProtocol(WebSocketServerProtocol):
    ...
    def onOpen(self):
        ...
        self.factory.communicate(self, "Connected to Onyx Network", True)
...
class OnyxRouletteFactory(WebSocketServerFactory):

    def __init__(self, *args, **kwargs):
        super(OnyxRouletteFactory, self).__init__(*args, **kwargs)
        self.clients = []
...
if __name__ == '__main__':
    ...
    factory = OnyxRouletteFactory(u"ws://127.0.0.1:9000")
    factory.protocol = OnyxServerProtocol
